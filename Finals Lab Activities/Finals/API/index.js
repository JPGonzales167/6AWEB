...
app.post("/api/books/AddBook", multer().none(), async (req, res) => {
  try {
    const numOfDocs = await database.collection("Books").countDocuments();    await database.collection("Books").insertOne({
...
// Update a book
app.put("/api/books/UpdateBook", express.json(), async (req, res) => {
  try {
